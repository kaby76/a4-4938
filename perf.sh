...
files2=`dotnet trglob './examples/**/*.cs' | grep -v '.errors$' | grep -v '.tree$'`
files=()
for f in $files2
do
...
if [[ "$type" == "group" ]]
then
    echo "${files[1]}" | dotnet trwdog ./bin/Debug/net10.0/Test.exe -x -prefix group > /dev/null 2>&1
    for ((i=1; i<=n; i++))
    do
