...
files2=`dotnet trglob './examples/**/*.cs' | tr -d '\r' | grep -v '[.]errors$' | grep -v '[.]tree$' | grep -v '[.]trq$'`
files=()
for f in $files2
do
...
assertions_err=0
for file in `dotnet trglob './examples/**/*.cs' | grep -v '[.]errors$' | grep -v '[.]tree$' | grep -v '[.]trq$'`
do
    trq=$file.trq
    if [ -f "$trq" ]
    then
